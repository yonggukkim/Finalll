...
<script>
	...
	function check() {
		 if (!document.frm.memberId.value) {
			...
			//("url, 자식창 이름,새창 옵션")
			openWin = window.open("idCheck","idCheckPage","width=570, height=350");
			
			/* openWin.getParentText(); */
...
<body id="page-top">
	<table align="center">
		<form:form name="frm" commandName="member" action="memberJoin"
			...
			<tr>
				<th>아이디 :</th>
				<td><form:input path="memberId" name="memberId" 
						onkeydown="inputIdChk()" id="pInput" /> <input type="button"
					onclick="check()" value="ID 중복 체크">
			</tr>
			<tr>
				<th>패스워드 :</th>
			...
			<tr>
				<th>생년월일 :</th>
				<td><form:input path="memberBirth"
						placeholder="99/99/99형식으로 입력해주세요" /></td>
			</tr>
			<tr>
				<th>성별 :</th>
				<td><form:radiobutton path="memberGender" value="M" label="남자" />
					<form:radiobutton path="memberGender" value="W" label="여자" /></td>
			</tr>
			...
			<tr>
				...
				<td><form:select path="memberCountryNum">
					...
					</form:select> <form:input path="memberPhone" /></td>
			</tr>
			<tr>
				<th>이메일 :</th>
			...
			<tr>
				<th>결혼 유무 :</th>
				<td><form:radiobutton path="memberMarrige" value="N" label="미혼" />
					<form:radiobutton path="memberMarrige" value="Y" label="기혼" /></td>
			</tr>
			<tr>
				<th>직업 :</th>

				<td><form:select path="memberJob">
			...
			<tr>
				<th>이벤트 유선전화 수신여부 :</th>
				<td><form:radiobutton path="memberAgreeTel" value="Y"
						label="동의합니다." /> <form:radiobutton path="memberAgreeTel"
						value="N" label="동의하지 않습니다." /></td>
			</tr>
			<tr>
				<td colspan="2"><button type="submit" class="btn"
						onclick="javascritp:checkValue();" style="margin-bottom: 20px">작성
						완료</button> <a href="memberTerms"><button type="button" class="btn"
							style="margin-bottom: 20px">취소</button></a></td>
			</tr>
		</form:form>
	</table>
</html>
