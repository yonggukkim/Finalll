...
<%
	Activitys actm = (Activitys) request.getAttribute("modify");
%>
<!DOCTYPE html>
<html lang="en">
...
<body id="page-top">
	<section id="services" class="bg-light">
		<div class="container">
			<div class="row">
				<form:form commandName="activity" action="activity_modify">
					...
					<!-- encType="multiplart/form-data" -->
						<thead>
							...
							<tr>
								...
								<td><%=actm.getStaffNumber()%></td>
								<th scope="col">등록일</th>
								<td><%=actm.getActivityRegdate()%></td>
								
							</tr>
						</thead>
						<tbody>
							 <tr>
								<form:hidden path="activityNum"
									value="<%=actm.getActivityNum()%>" />
								<form:hidden path="staffNumber"
									value="<%=actm.getStaffNumber()%>" />
								<th>상품명 :</th>
								<td><form:input path="activityName"
										value="<%=actm.getActivityName()%>" /></td>
								<th>상품분류 :</th>
								<td><form:input value="<%=actm.getActivityCate()%>"
										path="activityCate" />
							</tr>
							<tr>
								<th>상품내용 :</th>
								<td><form:textarea cols="30" path="activityContent"
										value="<%=actm.getActivityContent()%>" /></td>
							</tr>
							<!-- <tr>
								<th>첨부파일 :</th>
